...
class CurbDetectionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('grass_warning_threshold', 0.35) # 35% de pasto en un lado activa advertencia
    ...
    def image_callback(self, msg):
        # Convertir sensor_msgs/Image a formato OpenCV (BGR)
        try:
            height = msg.height
            width = msg.width
        ...
        # Crear Máscara de Césped/Pasto
        grass_mask = cv2.inRange(hsv, lower_green, upper_green)

        # Filtrado morfológico para eliminar ruido (pequeñas manchas)
        ...
        # 3. Análisis de Zonas de Seguridad (Izquierda, Centro, Derecha)
        ...
        status = "ON_PATH"
        steering_recommendation = 0.0 # 0.0 = Centrado, <0 = Girar Derecha, >0 = Girar Izquierda
        ...
        # Añadir Texto de Estado y Recomendación de Dirección
        color_status = (0, 255, 0) # Verde para ON_PATH
        ...
        # Publicar estado estructurado en JSON (para lectura en la MacBook)
        status_data = {
            "status": status,
